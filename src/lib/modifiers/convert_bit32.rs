...
use full_moon::{ast::{punctuated::Punctuated, Assignment, BinOp, Call, Do, Expression, FunctionArgs, FunctionCall, Index, LocalAssignment, Stmt, Suffix, UnOp, Var}, tokenizer::TokenReference, visitors::VisitorMut};
			...
			if let FunctionArgs::Parentheses {
				...
				let binop = match self {
					...
					Bit32Method::Not => {
						...
						return Some(ast_util::create_parentheses(masking_unop, Some(parentheses.clone())));
					}
				};

				let second_arg = iter.next()?;
				let bitop_exp = ast_util::create_binary_operator(
					first_arg.clone(),
					binop,
					second_arg.clone()
				);
				...
				return Some(ast_util::create_parentheses(masking_bin_exp, Some(parentheses.clone())));
			}
		}
		None
	}
...
impl Bit32Converter {
	...
	fn check_replaced(&mut self, variables: &Punctuated<Var>, expressions: &Punctuated<Expression>) -> bool {
						...
						if let Some(first) = first {
							...
							if let Suffix::Index(index) = first {
								...
								if let Some(index) = index {
									println!("debug index: {}, var: {}", index.trim(), var.to_string().trim());
									if let Ok(method) = Bit32Method::from_str(index.trim()) {
										self.bit32_methods.insert(var.to_string().trim().to_owned(), method);
